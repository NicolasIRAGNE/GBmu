...
Token<int, CustomStack &> *SymbolTerminalInteger::createToken(std::string tokenContent)
{
	int					index;
	t_integer_format	format;
	...
	if (tokenContent.length() && tokenContent.at(0) == '$')
		return new Token<int, CustomStack &>(*this, stoi("0x" + tokenContent.substr(1), 0, format));
	else
		return new Token<int, CustomStack &>(*this, stoi(tokenContent, 0, format));
}
